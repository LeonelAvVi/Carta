-- Incluye descuento activo en la cuenta del comensal
...
begin
  ...
  select
    ...
    count(*)::int
  into v_bill_subtotal, v_order_count
  from public.orders o
  where o.table_id = p_table_id
    and o.restaurant_id = p_restaurant_id
  ...
  into v_discount_amount, v_discount_description
  from public.tables t
  where t.id = p_table_id;

  v_bill_total := greatest(0, v_bill_subtotal - coalesce(v_discount_amount, 0));
  ...
  return jsonb_build_object(
    ...
    'bill_subtotal', v_bill_subtotal,
    'bill_total', v_bill_total,
    'discount_amount', coalesce(v_discount_amount, 0),
    'discount_description', v_discount_description,
...
comment on function public.get_guest_table_order_status(uuid, uuid) is
  'Carta pública: status, pedidos entregados, descuento y total a pagar.';
